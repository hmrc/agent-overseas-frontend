...
class BaseISpec
    ...
    with WireMockSupport with AuthStubs with DataStreamStubs with MetricsTestSupport with DefaultAwaitTimeout
    with MongoSupport {
  ...
  protected def containElement(id: String, tag: String, attrs: Map[String, String]): Matcher[Result] =
    new Matcher[Result] {
      override def apply(result: Result): MatchResult = {
